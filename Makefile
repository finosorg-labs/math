...
linux:
	...
	@rm -f $(LINUX_BUILD_DIR)/libfinkit_math_static_base.a
...
macos:
	...
	@echo "==> Cleaning intermediate build artifacts"
	@rm -f $(MACOS_BUILD_DIR)/libfinkit_math_static_base.a
...
bench:
	...
	@if [ -f $(LINUX_BUILD_DIR)/benchmarks/math_benchmarks ]; then \
		$(LINUX_BUILD_DIR)/benchmarks/math_benchmarks; \
	else \
...
clang-tidy:
	...
	@find math-c include \( -name '*.c' -o -name '*.h' \) \
		! -name 'platform_win.c' \
...
cppcheck:
	...
	@find math-c include \( -name '*.c' -o -name '*.h' \) -print | \
...
help:
	@echo "math Makefile - Build and Test Targets"
	@echo ""
	@echo "Build Targets:"
