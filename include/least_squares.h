 ...
 *       in hot paths, use fc_math_least_squares_work() with caller-provided workspace.
 */
int fc_math_least_squares(double* X, const double* y, size_t n, size_t p, double* beta);

/**
 * Solve ordinary least squares regression with caller-provided workspace.
 *
 * Same as fc_math_least_squares() but uses caller-provided workspace to avoid
 ...
 */
int fc_math_least_squares_work(
    double* X,
    const double* y,
    size_t n,
    size_t p,
 ...
 * than calling fc_math_least_squares() in a loop due to better cache usage.
 *
 ...
 */
size_t fc_math_least_squares_batch(
    double* X,
    const double* y,
    size_t n,
    size_t p,
 ...
 * @return 0 on success, negative error code on failure (same as fc_math_least_squares)
 *
